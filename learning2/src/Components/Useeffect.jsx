...
const Useeffect = () => {
    ...
    const [secound, setsecound] = useState(100)
    const [Color, setColor] = useState("red")
    function one() {
        setfirst(first + 1)
    }
    let two = () => {
        setsecound(secound + 10)
    }
    useEffect(() => {
    ...
    return (
        <>
        <CounterContext.Provider value={{first,setfirst,secound,setsecound,Color,setColor,one,two}}>
            <div className='flex mt-8 flex-col items-center'>
                <h1 className='text-3xl mt-8'>useEffect</h1>
                <h1 className='text-3xl mt-4'>{first}</h1>
                <h1 className='text-3xl mt-4'>{secound}</h1>
